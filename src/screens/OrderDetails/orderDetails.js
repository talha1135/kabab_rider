...
export default class OrderDetails extends Component {
  ...
  Picked(){
    // console.log("picked",this.state.data);

     Caller.callServer("orderPicked","POST", [this.state.data.id])
      .then( response  => {

        ToastAndroid.show('Order Picked', ToastAndroid.SHORT);
    ...
    Delivered(){
    // console.log("Delivered",this.state.data);

     Caller.callServer("orderDelivered","POST", [this.state.data.id])
      .then( response  => {
      ...
      <View style={styles.container}>
        ...
        >
          <View style={{paddingHorizontal: wp('4%'), paddingVertical:wp('2%')}}>
            ...
            <View style={{width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                ...
                {this.state.language=="eng" ? 
                  <View>
                    <Text style={{color:'#fff',fontSize:wp('4%'),textAlign:'center'}} > To Pickup - Order# {this.state.data.id} </Text>
                    <Text style={{color:'#fff',fontSize:wp('4%'),textAlign:'center'}} >{this.state.data.updated_at}</Text>
                  </View>
                  :
                  <View>
                    <Text style={{color:'#fff',fontSize:wp('4%'),textAlign:'center'}} > للالتقاط - طلب# {this.state.data.id} </Text>
                    <Text style={{color:'#fff',fontSize:wp('4%'),textAlign:'center'}} >{this.state.data.updated_at}</Text>
                  </View>
        ...
        <ScrollView style={{
          ...
          <View style={{
            ...
            <FlatList
              data={this.state.data.orders_item}
